...
class ContextMenuObject {
    ...
    addEntry(entry: React.ReactChild) {
...
class ContextMenu extends React.Component<{ context: ContextMenuObject, relativeElement?: HTMLElement, parent?: HTMLElement } & MultilayerContextProps, ContextMenuState>{
    ...
    updatePosition() {
        ...
        if (realX + width > window.innerWidth) {
            if (this.props.relativeElement) width += this.props.relativeElement.offsetWidth
            x -= width
        }

        if (realY + height > window.innerHeight) {
            if (this.props.relativeElement) height -= this.props.relativeElement.offsetHeight
            y -= height
        }
    ...
    render() {
        return (
            ...
            >
                {React.Children.map(this.props.context.entries, (element: ReactElement<ContextMenuEntry>) => {
                    if (element && typeof element.type != "string") {
                        if (element.type === ContextMenuEntry || element.type.prototype instanceof ContextMenuEntry) {
...
class ContextMenuEntry<Props = {}, State = {}> extends React.Component<Props & MultilayerContextProps & { sort?: number, parent?: ContextMenu }, State> { }
...
export class ContextLabel extends ContextMenuEntry {
    render() {
        ...
        return (<div
            style={Object.assign({}, defaultEntryStyles, {
...
export class ContextButton extends ContextMenuEntry<{ onClick: () => void }> {
    render() {
        ...
        return (<Div
            ...
            hoverStyle={Object.assign({}, defaultEntryStyles, {
                backgroundColor: 'rgba(0,0,0, 0.2)'
...
export class MultilayerWithContext extends Layer<MultilayerContextProps & { id: string }, MultilayerContextState> {
    constructor(props) {
        super(props)
        this.state = {
            visible: false,
    ...
    render() {
        return (
            <Multilayer id={this.props.id}>
                ...
                {this.props.children}
                {this.state.visible && <ContextDrawLayer id="ContextDrawLayer" context={this.state.context} {...this.props} />}
